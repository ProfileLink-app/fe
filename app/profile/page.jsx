...
export default function Profile() {
    const COLOR_CALC = +profileData.theme[0] * 0.299 + +profileData.theme[1] * 0.587 + +profileData.theme[2] * 0.114;
        ...
        <div style={{ backgroundColor: `#${profileData.theme}` }} className='min-h-screen'>
            ...
            <div className='relative top-0 flex flex-col h-full pb-10 mx-auto max-w-7xl'>
                <section className='flex flex-col items-center' style={COLOR_CALC > 186 ? { color: '#4B5563' } : { color: '#FFFFFF' }}>
                    ...
                    <h2 className='mb-10 text-md'>{profileData.bio}</h2>
                                ...
                                return (
                                    <a href={link.url} target='_blank' key={key}>
                                        <div className='flex items-center justify-between w-full gap-4 px-5 py-5 transition rounded-md cursor-pointer bg-gray-950 bg-opacity-40 hover:bg-opacity-60' style={COLOR_CALC <= 30 ? { color: '#4B5563', backgroundColor: '#FFFFFF' } : { color: '#FFFFFF' }}>
                                            ...
                                            <Image src='/openlink-black.svg' width='0' height='0' sizes='100%' className='w-5 h-5' />
                                        </div>
                                    </a>
                                );
                            })}
